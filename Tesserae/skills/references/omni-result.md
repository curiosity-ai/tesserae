...
- `.MarkAsViewed(bool viewed = true)` / `IsViewed` — the row the user just opened. Its title (with
  the identifier and chevron, when `SetId` gave it one) turns purple and stays that way, and marking it plays one short pulse over the whole card so it is
