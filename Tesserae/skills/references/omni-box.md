...
- `.IsGenerating` (bool) — toggles the footer spinner + elapsed-time indicator and swaps the send button for a stop button. The send button is quiet while the input is empty and takes the primary colour once there is text; both changes, and the send/stop swap, animate briefly (off under `prefers-reduced-motion`). `.GeneratingText` (string) — read/write the indicator label; setting it updates the footer live. `.AllowSendWhileGenerating` (bool) — read/write the config flag above.
