...
namespace Tesserae
{
    ...
    public class SavingToast : IComponent
    {
        ...
        public void Saving(string message = null)
        {
            ...
            var messageText = TextBlock(message ?? _initialMessage);
            var progress = ProgressIndicator().Indeterminated();
            var content = VStack().Children(messageText, progress).Style(s => s.gap = "8px");

            _toast.Information(title, content);
        }
