...
namespace Tesserae
{
    ...
    /// </summary>
    /// <remarks>
    /// This is the reference-keyed counterpart to <see cref="ObservableStack"/>. Use
    /// <see cref="ReconcilingStack{T}"/> when you have a list of data models with stable instance
    /// identity (matched by reference), want each row built lazily from a factory, and want matched
    /// rows preserved across updates. Use <see cref="ObservableStack"/> instead when the source
    ...
    /// you need reordering or content-hash-driven re-rendering. <see cref="ReconcilingStack{T}"/> only
    ...
    /// </remarks>
    [H5.Name("tss.ReconcilingStack")]
    public sealed class ReconcilingStack<T> : IComponent where T : class
    {
        ...
        public ReconcilingStack(IObservable<IReadOnlyList<T>> source, Func<T, IComponent> renderItem, Stack host = null)
        {
