...
namespace Tesserae
{
    ...
    /// their latest values.
    /// </summary>
    [Transpose.Name("tss.CombinedObservableT4")]
    public sealed class CombinedObservable<T1, T2, T3, T4> : IObservable<(T1 first, T2 second, T3 third, T4 forth)>
    {
        ...
        /// the callback.
        /// </summary>
        public IDisposable Subscribe(Action<(T1 first, T2 second, T3 third, T4 forth)> callback, bool fireImmediately = true)
        {
            ObservableEvent.ValueChanged<(T1 first, T2 second, T3 third, T4 forth)> handler = v => callback(v);
