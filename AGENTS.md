...
## Icon fonts
- The UIcons webfonts, the `uicons-*.css` files and `Tesserae/src/Icons/UIcons.cs` are all generated by `Build.UpdateInterfaceIcons`; never edit them by hand.
- The same project then measures every glyph in headless Chromium (Playwright) and bakes the optical centering into the glyph outlines of the woff2 files, editing the fonts in C# (`Woff2File.cs`, `TransformedGlyf.cs`) with no external tooling. There is no adjustment stylesheet: a `position: relative` offset gets rounded to a whole CSS pixel, which does nothing at Tesserae's default 13px icon size.
