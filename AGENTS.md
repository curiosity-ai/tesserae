...
## Icon fonts
- The UIcons webfonts and `Tesserae/src/Icons/UIcons.cs` are generated by `Build.UpdateInterfaceIcons`.
- `Build.UIconsOpticalCentering` renders every glyph in headless Chromium (Playwright), measures how far it is from being optically centred in its layout box, and generates `Tesserae/tps/assets/css/tss.uicons.adjustments.css`. Regenerate with `dotnet run --project Build.UIconsOpticalCentering`; never edit that stylesheet by hand, and re-run it after bumping the icon set, since UIcons codepoints change every release.
