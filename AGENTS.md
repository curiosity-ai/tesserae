...
- The same project then measures every glyph in headless Chromium (Playwright) and bakes the optical centering into the glyph outlines of the woff2 files, via `centre-uicons-outlines.py` (needs `pip install fonttools brotli`). There is no adjustment stylesheet: a `position: relative` offset gets rounded to a whole CSS pixel, which does nothing at Tesserae's default 13px icon size.
