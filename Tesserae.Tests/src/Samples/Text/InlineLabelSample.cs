    ...
    public class InlineLabelSample : IComponent, ISample
    {
        ...
        public InlineLabelSample()
        {
            _content = SectionStack().Secondary()
                ...
                .FlatSection(VStack().WS().Children(FromAModel()))
                .SeeAlso(typeof(OmniResultSample), typeof(DetailsGridSample), typeof(BadgeSample), typeof(ButtonSample), typeof(AIVariantsSample));
        }
        ...
        private IComponent FromAModel()
        {
            return FeatureCard("Facts a model supplied", "AI()",
                "AI() marks a label as something a model produced - a suggested tag, a confidence, \"summarised by AI\". It takes the purple-to-blue tint, the accent border and, unless it already has a mark, the Sparkles glyph. A label that already carries a source keeps its own mark: an avatar or a logo says where the fact came from more precisely than Sparkles would.",
                HStack().WS().Wrap().Gap(8.px()).AlignItemsCenter().PT(8).PB(8).Children(
                    InlineLabel("Summarised by AI").AI(),
                    InlineLabel("94% confidence").AI(withSparklesIcon: false),
                    InlineLabel("Suggested tag: brakes").AI(),
                    InlineLabel().AI().Tooltip("Generated by a model"),
                    InlineLabel("Drafted from 12 sources").AI().OnClick(_ => Toast().Information("Opening the 12 sources"))),
                TextBlock("In a footer the pill's chrome comes off for every label, so an AI one keeps only its colour - a fact in a line of facts rather than a chip among plain type:").Small().MT(8).MB(8),
                OmniResult("ai-footer-sample", "Line 3 brake sensor failures")
                    .SetIcon("AI", Theme.Colors.Purple600)
                    .SetText("Eleven of the last fourteen runs failed, every one of them after a mount torque below 12 Nm.")
                    .SetFooterEntries(
                        InlineLabel("Summarised by AI").AI(),
                        InlineLabel("12 sources").AI(withSparklesIcon: false),
                        InlineLabel("Apr 12, 2024")));
        }
