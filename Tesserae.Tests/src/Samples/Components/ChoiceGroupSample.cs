    ...
    public class ChoiceGroupSample : IComponent, ISample
    {
        ...
        public ChoiceGroupSample()
        {
            _content = SectionStack().Secondary()
               .SampleTitle(typeof(ChoiceGroupSample), UIcons.List, "A control to select a single option from a list")
               .FlatSection(Stack().Children(
                    ...
                    TextBlock("They emphasize all options equally, which can be useful when you want to ensure the user considers all available alternatives before making a selection."))).SetTitle("Overview")))
               .FlatSection(Stack().Children(
                    Card(VStack().WS().Children(
                    TextBlock("Use ChoiceGroups when there are between 2 and 7 options and screen space is available. For more than 7 options, a Dropdown is typically more efficient. List options in a logical order (e.g., most likely to least likely). Align options vertically whenever possible for better readability and localization support. Always provide a default selection if one is significantly more likely than the others, and ensure the safest option is the default if applicable."))).SetTitle("Best Practices")))
               .FlatSection(Stack().Children(
                    Card(VStack().WS().Children(
