    ...
    public class SplitViewSample : IComponent, ISample
    {
        ...
        public SplitViewSample()
        {
            ...
            _content = SectionStack().Secondary()
               .S()
               .SampleTitle(typeof(SplitViewSample), UIcons.Apps, "A component to split the view")
               .FlatSection(Stack().Children(
                    ...
                    TextBlock("Tesserae provides both 'SplitView' (vertical split) and 'HorizontalSplitView' with support for resizable handles and initial sizing."))).SetTitle("Overview")))
               .FlatSection(Stack().Children(
                    Card(VStack().WS().Children(
                    TextBlock("Use SplitViews when users need to see two related sets of content at the same time. Enable resizability if the ideal balance between the two panels depends on the user's task or screen size. Set sensible minimum and maximum sizes for the panels to prevent them from disappearing or becoming too large. Use distinct background colors or borders to help users distinguish between the two areas."))).SetTitle("Best Practices")))
               .FlatSection(
                    VStack().S().Children(
                        Card(VStack().WS().Children(
