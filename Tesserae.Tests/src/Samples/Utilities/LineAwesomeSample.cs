    ...
    public class LineAwesomeSample : IComponent, ISample
    {
        ...
        private class IconItem : ISearchableItem
        {
            ...
            public IconItem(LineAwesome icon, string name)
            {
                ...
                component = HStack().Children(Icon(icon, size: TextSize.Large).MinWidth(36.px()).AlignCenter(), TextBlock($"{name}").Title(icon.ToString()).Wrap().AlignCenter()).PaddingBottom(4.px());
            }
